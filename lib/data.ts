...
import type { AdminOverview, BusinessEvent, Client, ClientOverview, Credential, DiscoveredWorkflowQueue, Invoice, N8nInstance, Onboarding, Payment, ProviderBalance, Subscription, SupportTicket, Workflow } from "@/lib/types";

const safe = <T,>(value: T[] | null | undefined): T[] => value ?? [];
...
export async function adminOverview(): Promise<AdminOverview> {
  ...
  const cs = safe(clients.data) as Client[];
  const ss = safe(subs.data) as Subscription[];
  const is = safe(invoices.data) as Invoice[];
  const ps = safe(payments.data) as Payment[];
  const ts = safe(tickets.data) as SupportTicket[];
  const ws = safe(workflows.data) as Workflow[];
  const ns = safe(infra.data) as N8nInstance[];
  const ons = safe(onboarding.data) as Onboarding[];
  const creds = safe(credentials.data) as Credential[];
  const bals = safe(balances.data) as ProviderBalance[];
  ...
  const attention = buildAttentionItems({invoices:is,workflows:ws,infrastructure:ns,tickets:ts,onboarding:ons,credentials:creds,balances:bals});
...
export async function clientOverview(clientId:string): Promise<ClientOverview> {
  ...
  const trackedCounts=ev.reduce<Record<string,number>>((acc,e)=>{acc[e.event_type]=(acc[e.event_type]||0)+1;return acc;},{});
  const now=Date.now(), last30=ev.filter(e=>now-new Date(e.occurred_at).getTime()<=30*86400000), prev30=ev.filter(e=>now-new Date(e.occurred_at).getTime()>30*86400000&&now-new Date(e.occurred_at).getTime()<=60*86400000);
  const lastCount=last30.length, prevCount=prev30.length;
  ...
  const failing=wf.filter(w=>w.status==="error"||w.actual_state!==w.desired_state).length;
  if(failing>0) opportunities.push({label:"Automation attention",detail:`${failing} client-visible automation record${failing===1?"":"s"} need review.`,kind:"Derived"});
  return {workflows:wf,events:ev,reports:safe(reports.data) as ClientOverview["reports"],subscriptions:safe(subscriptions.data) as Subscription[],invoices:safe(invoices.data) as Invoice[],payments:safe(payments.data) as Payment[],tickets:safe(tickets.data) as SupportTicket[],controls:safe(controls.data) as ClientOverview["controls"],trackedCounts,trend,opportunities};
}

export async function discoveredWorkflowQueue(): Promise<DiscoveredWorkflowQueue>{
  const s=await createClient();
  const [discovered,clients,connections,templates,instances]=await Promise.all([
  ...
  return {discovered:safe(discovered.data) as DiscoveredWorkflowQueue["discovered"],clients:safe(clients.data) as DiscoveredWorkflowQueue["clients"],connections:safe(connections.data) as DiscoveredWorkflowQueue["connections"],templates:safe(templates.data) as DiscoveredWorkflowQueue["templates"],instances:safe(instances.data) as DiscoveredWorkflowQueue["instances"]};
}
